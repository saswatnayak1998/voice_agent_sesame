...
def sample_topk(logits: torch.Tensor, topk: int, temperature: float):
    # Convert temperature to a tensor with the same dtype and device as logits
    temperature_tensor = torch.tensor(temperature, dtype=logits.dtype, device=logits.device)
...
class Model(nn.Module):
    ...
    def generate_frame(
        ...
        dtype = torch.bfloat16 if self.backbone.weight.dtype == torch.bfloat16 else torch.float16

        h = self.backbone(h, input_pos=input_pos, mask=curr_backbone_mask)
        h = h.to(dtype)  # ✅ Ensure dtype consistency
